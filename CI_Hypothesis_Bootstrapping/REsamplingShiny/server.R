  ...
  output$plot <- renderPlot({
    ...
    if (input$checkbox) {
      ...
      for (i in 1:reps) {
        ...
        restats[i] <<- stat(sample(x,n,replace=TRUE),input$statistic)
      }
    }
    ...
    restatsTrim <- restatsTrim[ restatsTrim>qtrim[1] & restatsTrim<qtrim[2] ]
    ...
    xb1RE <- signif(mean(restats),3)
    s1RE <- signif(sd(restats),3)
    ...
    REtitleLabel <- switch(input$statistic,
                         mean = bquote(bold(Resample~Means)~(list(mu[bar(x)]%~~%.(xb1RE),~sigma[bar(x)]%~~%.(s1RE)))),
                         median = bquote(bold(Resample~Medians)~(list(mu[median]%~~%.(xb1RE),~sigma[median]%~~%.(s1RE)))),
                         stdev = bquote(bold(Resample~Std~Devs)~(list(mu[s]%~~%.(xb1RE),~sigma[s]%~~%.(s1RE)))),
                         var = bquote(bold(Resample~Variances)~(list(mu[s^2]%~~%.(xb1RE),~sigma[s^2]%~~%.(s1RE)))),
                         max = bquote(bold(Resample~Max)~(list(mu[max]%~~%.(xb1RE),~sigma[max]%~~%.(s1RE)))),
                         min = bquote(bold(Resample~Min)~(list(mu[min]%~~%.(xb1RE),~sigma[min]%~~%.(s1RE)))))
    ...
    ### plot resample statistics
    
    
    
    if (input$statistic=='mean'){
      ci = round(unname(t.test(xrm)$conf.int),2);
      xlabstr = paste('t: ',toString(ci));
      ciBoot = round(unname(quantile( restats, c(.025,.975))), 2);
      xlabstr = paste(xlabstr, " Boot: ");
      xlabstr = paste(xlabstr,toString(ciBoot));
    } else { 
      ciBoot = round(unname(quantile( restats, c(.025,.975))), 2);
      xlabstr = paste(" Boot: ",toString(ciBoot));
    }
    
    nhre<-hist(restatsTrim,
             breaks=breaks_mh,
             main=REtitleLabel,
             warn.unused = FALSE,
             col=tcol,
