...
```{r message=FALSE}
require(HH) # the 'car' package also has a vif() function I think
vif(fit)
```
...
```{r}
ci <- confint(fit2,level=.9)
ci
```

With 95% confidence, the mean job proficiency score increases by `r round(ci[4],2)` to `r round(ci[8],2)` points for each 1 unit increase in the aptitude score $x_3$.
...
pred <- predict.lm(fit2,newdata,interval="prediction")
pred
```
...
For Method 1: $\widehat{y}=44.76 + 0.122 x_4$

For Method 2: $\widehat{y}=(44.76+59.93) + (0.122+1.780) x4 = 104.69 + 1.902 x_4$

For Method 3: $\widehat{y}=(44.76+4.23) + (0.122+0.304) x4 =48.99 + 0.426 x_4$
